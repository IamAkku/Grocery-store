...
const ProductProp = props => {
    ...
    // NEED TO .DEFAULT THIS IN ORDER FOR WEBPACK TO RETURN THE CORRECT PATH
...
export default class ProductPage extends React.Component {
    ...
    componentDidMount() {
        axios.get("http://localhost:8080/product/")
            .then(res => {
                // Filter the array by type(vegetables, fruits etc)
                // I think it is probably better to do this on the backend via custom queries
                let array = (this.props.type === "beauty") ? Array.from(res.data) : Array.from(res.data).filter(element => element.type === this.props.type);
                this.setState({
                    products : array
                })
            })
